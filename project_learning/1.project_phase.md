...
future optmization
| **Current Project Flow (Keep as-is for now)**                 | **Future Production Flow (Optional Refactor Later)**    |
| ------------------------------------------------------------- | ------------------------------------------------------- |
| `run_local_loader.py` → Extract from Postgres + save RAW JSON | `run_extraction.py` → Extract only from Postgres        |
| `run_snowflake_loader.py` → RAW JSON → Snowflake RAW          | `run_local_loader.py` → Save extracted data to RAW JSON |
| `run_dbt_run.py` → Build Staging → Intermediate → Marts       | `run_snowflake_loader.py` → RAW JSON → Snowflake RAW    |
| `run_dbt_snapshot.py` → SCD Type 2 snapshots                  | `run_dbt_run.py` → Build Staging → Intermediate → Marts |
| `run_dbt_test.py` → Data quality validation                   | `run_dbt_snapshot.py` → SCD Type 2 snapshots            |
| **Total: 5 Airflow Tasks**                                    | `run_dbt_test.py` → Data quality validation             |
|                                                               | **Total: 6 Airflow Tasks**                              |
